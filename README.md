...
2. **녹화**를 누르거나 `Ctrl+Alt+F8`을 누릅니다. 필요하면 먼저 녹화 전 카운트다운을 지정합니다.
3. 마우스와 키보드를 사용한 뒤 같은 단축키 또는 **정지**로 녹화를 끝냅니다.
4. 반복 횟수, 무한 반복, 속도, 반복 간격을 선택하고 **재생**을 누릅니다.
5. **저장**으로 `.mumacro` JSON 매크로를 보관하고, **불러오기**로 다음 실행 때 재사용합니다.
